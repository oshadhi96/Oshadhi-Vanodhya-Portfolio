        ...
        <div className="relative rounded-full border border-white/10 bg-[#020617]/80 backdrop-blur-xl shadow-[0_4px_20px_-1px_rgba(0,0,0,0.3)] px-6 py-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              >
                <span className="h-8 w-8 rounded-full bg-gradient-to-br from-violet-400 to-indigo-500 flex items-center justify-center text-white font-bold text-sm shadow-inner shadow-white/20">O</span>
                <span className="hidden sm:inline-block font-serif italic text-lg">Oshi Vanodhya</span>
              </Link>
            </div>
            ...
            <div className="hidden lg:flex items-center gap-6">
              <ul className="flex items-center gap-6 list-none m-0 p-0">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    {link.href.startsWith("/#") && link.name !== "Speaking & Journal" ? (
                      <Link
                      ...
                      <Link
                        to={link.name === "Speaking & Journal" ? "/journal" : link.href}
                        className="text-sm font-medium text-slate-300 hover:text-violet-400 transition-colors relative group inline-block py-3 px-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 rounded-sm"
                        onClick={(e) => {
              ...
              <div className="flex items-center gap-3">
                ...
                */}
                <Button asChild size="sm" className="rounded-full bg-white text-slate-900 hover:bg-violet-50 hover:text-violet-900 font-medium px-6 focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">
            ...
            <div className="lg:hidden flex items-center gap-4">
              <Button asChild size="sm" className="rounded-full bg-white text-slate-900 hover:bg-violet-50 hover:text-violet-900 font-medium px-4 h-8 text-xs focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">
                  <Link to="/contact">Contact</Link>
              </Button>
              <button
          ...
          >
            <div className="flex flex-col gap-6">
                ...
                {navLinks.map((link) => (
                  <li key={link.name}>
                    {link.href.startsWith("/#") ? (
                      <a
                        href={link.name === "Work" ? "/work" : link.name === "Testimonials" ? "/#kind-words" : link.href}
                        className="text-2xl font-serif italic text-slate-300 hover:text-violet-400 block py-2 border-b border-white/5"
                        onClick={(e) => {
                          setIsOpen(false);
                          const targetHref = link.name === "Work" ? "/work" : link.name === "Testimonials" ? "/#kind-words" : link.href;
              ...
              <div className="flex flex-col gap-4 mt-4">
                ...
                */}
                <Button asChild className="w-full rounded-full bg-violet-600 text-white hover:bg-violet-700 h-12 text-lg">
